...
use StoreEngine\Stripe\StripeObject;
...
abstract class Util
{
    ...
    {
        $types = 'v1' === $apiMode ? \StoreEngine\Stripe\Util\ObjectTypes::mapping
            : \StoreEngine\Stripe\Util\ObjectTypes::v2Mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object'])
                ...
                if ('v2' === $apiMode && ('v2.core.event' === $resp['object'])) {
                    $eventTypes = \StoreEngine\Stripe\Util\EventTypes::thinEventMapping;
                    if (\array_key_exists('type', $resp) && \array_key_exists($resp['type'], $eventTypes)) {
                        $class = $eventTypes[$resp['type']];
                    } else {
                        $class = \StoreEngine\Stripe\V2\Event::class;
                    }
                }
                ...
                $class = \StoreEngine\Stripe\V2\Collection::class;
            } else {
                $class = \StoreEngine\Stripe\StripeObject::class;
            }
        ...
        foreach ($properties as $key => $property) {
            if (\array_key_exists($property->getName(), $json)) {
                if ('related_object' === $property->getName()) {
                    $related_object = new \StoreEngine\Stripe\RelatedObject();
                    ...
                    $reason = new \StoreEngine\Stripe\Reason();
    ...
    {
        if ($h instanceof \StoreEngine\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
