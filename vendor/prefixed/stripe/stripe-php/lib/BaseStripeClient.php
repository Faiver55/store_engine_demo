...
use StoreEngine\Stripe\Util\Util;
...
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    const DEFAULT_CONFIG = [
        ...
        'stripe_account' => null,
        'stripe_context' => null,
        'stripe_version' => \StoreEngine\Stripe\Util\ApiVersion::CURRENT,
    ...
    /** @var \StoreEngine\Stripe\Util\RequestOptions */
    private $defaultOpts;
    ...
    public function __construct($config = [])
    {
        if (\is_string($config)) {
            ...
            throw new \StoreEngine\Stripe\Exception\InvalidArgumentException('$config must be a string or an array');
        }
        ...
        $this->defaultOpts = \StoreEngine\Stripe\Util\RequestOptions::parse([
    ...
    public function request($method, $path, $params, $opts)
    {
        ...
        $apiMode = \StoreEngine\Stripe\Util\Util::getApiMode($path);

        $opts = $defaultRequestOpts->merge($opts, true);

        $baseUrl = $opts->apiBase ?: $this->getApiBase();
        $requestor = new \StoreEngine\Stripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl, $this->getAppInfo());
        list($response, $opts->apiKey) = $requestor->request($method, $path, $params, $opts->headers, $apiMode, ['stripe_client']);
        $opts->discardNonPersistentHeaders();
        $obj = \StoreEngine\Stripe\Util\Util::convertToStripeObject($response->json, $opts, $apiMode);
        if (\is_array($obj)) {
            // Edge case for v2 endpoints that return empty/void response
            // Example: client->v2->billing->meterEventStream->create
            $obj = new \StoreEngine\Stripe\StripeObject();
        }
     ...
     *
     * @return \StoreEngine\Stripe\ApiResponse
     */
    public function rawRequest($method, $path, $params = null, $opts = [])
    {
        ...
        $baseUrl = $opts->apiBase ?: $this->getApiBase();
        $requestor = new \StoreEngine\Stripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl);
     ...
     * @param array|\StoreEngine\Stripe\Util\RequestOptions $opts the special modifiers of the request
     *
     * @return \StoreEngine\Stripe\Collection|\StoreEngine\Stripe\V2\Collection of ApiResources
     */
    public function requestCollection($method, $path, $params, $opts)
    {
        ...
        $apiMode = \StoreEngine\Stripe\Util\Util::getApiMode($path);
        if ('v1' === $apiMode) {
            if (!($obj instanceof \StoreEngine\Stripe\Collection)) {
                $received_class = \get_class($obj);
                $msg = "Expected to receive `Stripe\\Collection` object from Stripe API. Instead received `{$received_class}`.";

                throw new \StoreEngine\Stripe\Exception\UnexpectedValueException($msg);
            }
            $obj->setFilters($params);
        } else {
            if (!($obj instanceof \StoreEngine\Stripe\V2\Collection)) {
                $received_class = \get_class($obj);
    ...
    {
        $obj = $this->request($method, $path, $params, $opts);
        if (!($obj instanceof \StoreEngine\Stripe\SearchResult)) {
            $received_class = \get_class($obj);
    ...
    /**
     * @param \StoreEngine\Stripe\Util\RequestOptions $opts
     *
     * @throws \StoreEngine\Stripe\Exception\AuthenticationException
     *
     * @return string
     */
    private function apiKeyForRequest($opts)
    {
        ...
        if (null === $apiKey) {
            ...
            throw new \StoreEngine\Stripe\Exception\AuthenticationException($msg);
        }
     ...
     *
     * @throws \StoreEngine\Stripe\Exception\InvalidArgumentException
     */
    private function validateConfig($config)
    {
        // api_key
        if (null !== $config['api_key'] && !\is_string($config['api_key'])) {
            throw new \StoreEngine\Stripe\Exception\InvalidArgumentException('api_key must be null or a string');
        }

        if (null !== $config['api_key'] && ('' === $config['api_key'])) {
            $msg = 'api_key cannot be the empty string';

            throw new \StoreEngine\Stripe\Exception\InvalidArgumentException($msg);
        }
        ...
        if (null !== $config['client_id'] && !\is_string($config['client_id'])) {
            throw new \StoreEngine\Stripe\Exception\InvalidArgumentException('client_id must be null or a string');
        }

        // stripe_account
        if (null !== $config['stripe_account'] && !\is_string($config['stripe_account'])) {
            throw new \StoreEngine\Stripe\Exception\InvalidArgumentException('stripe_account must be null or a string');
        }

        // stripe_context
        if (null !== $config['stripe_context'] && !\is_string($config['stripe_context'])) {
            throw new \StoreEngine\Stripe\Exception\InvalidArgumentException('stripe_context must be null or a string');
        }

        // stripe_version
        if (null !== $config['stripe_version'] && !\is_string($config['stripe_version'])) {
            throw new \StoreEngine\Stripe\Exception\InvalidArgumentException('stripe_version must be null or a string');
        }

        // api_base
        if (!\is_string($config['api_base'])) {
            throw new \StoreEngine\Stripe\Exception\InvalidArgumentException('api_base must be a string');
        }

        // connect_base
        if (!\is_string($config['connect_base'])) {
            throw new \StoreEngine\Stripe\Exception\InvalidArgumentException('connect_base must be a string');
        }

        // files_base
        if (!\is_string($config['files_base'])) {
            throw new \StoreEngine\Stripe\Exception\InvalidArgumentException('files_base must be a string');
        }

        // app info
        if (null !== $config['app_info'] && !\is_array($config['app_info'])) {
            throw new \StoreEngine\Stripe\Exception\InvalidArgumentException('app_info must be an array');
        }
        ...
        if (!empty($extraConfigKeys)) {
            ...
            throw new \StoreEngine\Stripe\Exception\InvalidArgumentException('Found unknown key(s) in configuration array: ' . $invalidKeys);
        }
    }
     ...
     * @param 'v1'|'v2' $apiMode
     *
     * @return \StoreEngine\Stripe\StripeObject
     * */
    public function deserialize($json, $apiMode = 'v1')
    {
        return \StoreEngine\Stripe\Util\Util::convertToStripeObject(\json_decode($json, true), [], $apiMode);
    }
     ...
     *
     * @return \StoreEngine\Stripe\ThinEvent
     */
    public function parseThinEvent($payload, $sigHeader, $secret, $tolerance = Webhook::DEFAULT_TOLERANCE)
    {
        ...
        try {
            return Util::json_decode_thin_event_object(
                $eventData,
                '\StoreEngine\Stripe\ThinEvent'
            );
        } catch (\ReflectionException $e) {
            // Fail gracefully
            return new \StoreEngine\Stripe\ThinEvent();
        }
    }
}
