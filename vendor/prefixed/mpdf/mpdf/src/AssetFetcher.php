...
use StoreEngine\Mpdf\File\LocalContentLoaderInterface;
use StoreEngine\Mpdf\File\StreamWrapperChecker;
use StoreEngine\Mpdf\Http\ClientInterface;
use StoreEngine\Mpdf\Log\Context as LogContext;
use StoreEngine\Mpdf\PsrHttpMessageShim\Request;
use StoreEngine\Mpdf\PsrLogAwareTrait\PsrLogAwareTrait;
use StoreEngine\Psr\Log\LoggerInterface;

class AssetFetcher implements \StoreEngine\Psr\Log\LoggerAwareInterface
{
	...
	public function fetchDataFromPath($path, $originalSrc = null)
	{
		...
		if ($wrapperChecker->hasBlacklistedStreamWrapper($path)) {
			throw new \StoreEngine\Mpdf\Exception\AssetFetchingException('File contains an invalid stream. Only ' . implode(', ', $wrapperChecker->getWhitelistedStreamWrappers()) . ' streams are allowed.');
		}
	...
	public function fetchRemoteContent($path)
	{
		...
		try {
			...
			/** @var \StoreEngine\Mpdf\PsrHttpMessageShim\Response $response */
			...
			if (!str_starts_with((string) $response->getStatusCode(), '2')) {
				...
				if ($this->mpdf->debug) {
					throw new \StoreEngine\Mpdf\MpdfException($message);
				}
			...
			if ($this->mpdf->debug) {
				throw new \StoreEngine\Mpdf\MpdfException($message, 0, E_ERROR, null, null, $e);
			}
